        ...
        F: Fn(u32) -> bool,
    {
        ...
        if num_hex == exp_chars {
            let uval = u32::from_str_radix(&hexs, 16)
                .expect("We've just scanned for exp_char hex chars of input");
            ...
            match char::from_u32(uval) {
                ...
                None => {
                    eprintln!(
                        "{}:{}:{} - error - universal character name \\{}{} does not map to a char",
                        self.location.f(),
                        self.location.l(),
                        self.location.c(),
                        esc_char,
...
impl<'input, C> UnivEsc for UnivEscImpl<'input, C> {
    fn consume_universal_short(&self) {
        match self.text.peek() {
            ...
            _ => panic!(
                "{}:{}:{} - FATAL - this isn't a short universal escape",
                self.location.f(),
                self.location.l(),
                self.location.c()
            ),
